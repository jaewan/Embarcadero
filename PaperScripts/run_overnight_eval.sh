#!/bin/bash
# PaperScripts/run_overnight_eval.sh
#
# Paper-replication overnight sweep — forked from scripts/run_overnight_eval.sh
# with publication-honest defaults (see PaperScripts/README.md).
#
# Shared infrastructure still lives under scripts/ (run_multiclient, cluster_setup,
# latency helpers). This file owns paper cell matrix + knobs only.
#
# Improvements vs scripts/ copy:
#   - E2/E9 throughput cells set RUNTIME_MODE=throughput (not misleading latency)
#   - Default CLIENT_PUB_BATCH_KB=2048 for throughput (matches client.yaml design)
#   - Results under data/paper_eval/ by default
#   - Requires join-watchdog fix (commit 9c6aea99+) so short-run Bandwidth ≈ Send-done
#
# Usage:
#   bash PaperScripts/run_overnight_eval.sh
#   SMOKE=1 bash PaperScripts/run_overnight_eval.sh
#   SKIP_BASELINES=1 bash PaperScripts/run_overnight_eval.sh
#   bash PaperScripts/run_e2_throughput_matrix.sh   # waits for idle, then E2 N=1 matrix
#
# Detached:
#   nohup bash PaperScripts/run_overnight_eval.sh > /tmp/paper_eval_\$(date -u +%Y%m%dT%H%M%SZ).log 2>&1 &
#
# Key env overrides: same as scripts/run_overnight_eval.sh, plus
#   CLIENT_PUB_BATCH_KB=N   publish batch KB for TP cells (default 2048)
#   EPOCH_US_THROUGHPUT=N   ORDER=5 epoch µs for TP (default 500)
#   THREADS_THROUGHPUT=N    threads/broker for TP (default 6)
...
# Shared launchers / libs remain in scripts/; paper knobs live here.
SCRIPTS_DIR="$PROJECT_ROOT/scripts"
SCRIPT_DIR="$SCRIPTS_DIR"  # helpers below historically used SCRIPT_DIR for scripts/
cd "$PROJECT_ROOT"
...
# still defaults to 512 unless we pass this through.
...
OUT_BASE="${OUT_BASE:-$PROJECT_ROOT/data/paper_eval/$RUN_TAG}"
    ...
    for nc in 2 3; do
        ...
        if [[ "$SKIP_BASELINES" != "1" ]]; then
            run_multi_cell "e2_corfu_rf0_n${nc}" "$nc" "$local_csv" \
                ...
                TEST_TYPE=5 EMBARCADERO_RUNTIME_MODE=throughput \
                EMBARCADERO_CLIENT_PUB_BATCH_KB="$CLIENT_PUB_BATCH_KB" \
                EMBARCADERO_CORFU_SEQ_IP="$BROKER_IP_MULTI" \
                EMBARCADERO_HEAD_ADDR="$BROKER_IP"
