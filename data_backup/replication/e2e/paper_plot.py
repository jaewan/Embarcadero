...
def plot_bandwidth_vs_message_size(filename):
    ...
    for i, rep_factor in enumerate(replication_factors):
        ...
        plt.plot(message_sizes, avg_bandwidth, marker='o',
                 label=f'Num replica {rep_factor}',
                 color=color_palette_ack0[i])

    # Set plot labels and title with enhanced formatting
    plt.xlabel('Message Size', fontsize=14)  # Increase font size for labels
    plt.ylabel('Average e2eBandwidthMBps', fontsize=14)
    #plt.title('Average Bandwidth  Message Size for Different Configurations', fontsize=16)
...
# Example usage
plot_bandwidth_vs_message_size('disk_result') 
plot_bandwidth_vs_message_size('result') 
