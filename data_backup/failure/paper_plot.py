...
# Increased font sizes for column format
LABEL_FONTSIZE = 16       # Increased from 12
TITLE_FONTSIZE = 18       # Added title font size
TICKS_FONTSIZE = 14       # Increased from 10
LEGEND_FONTSIZE = 12      # Increased from 10
LINE_WIDTH = 2.0          # Increased from 1.5 for better visibility
GRID_ALPHA = 0.6
GRID_LINESTYLE = ':'
DPI = 300
...
def plot_real_time_throughput(csv_filename, output_prefix, event_filename=None):
    ...
    try:
        ...
        # To ensure fonts are large enough for a column in a paper
        plt.rcParams.update({
            'font.size': TICKS_FONTSIZE,  # Base font size
            'axes.titlesize': TITLE_FONTSIZE,
            'axes.labelsize': LABEL_FONTSIZE,
            'xtick.labelsize': TICKS_FONTSIZE,
            'ytick.labelsize': TICKS_FONTSIZE,
            'legend.fontsize': LEGEND_FONTSIZE,
            'legend.title_fontsize': LEGEND_FONTSIZE + 2,
        ...
        if event_data is not None:
             ...
             for index, event in event_data.iterrows():
                ...
                if event_ts_sec >= 0:
                     ...
                     plt.axvline(x=event_ts_sec, color=line_color, linestyle=linestyle, linewidth=1.5, alpha=0.7, label=line_label, zorder=1)


        # --- Customize Plot Appearance ---
        plt.xlabel('Time (seconds)', fontsize=LABEL_FONTSIZE, fontweight='bold')
        plt.ylabel('Throughput (GB/s)', fontsize=LABEL_FONTSIZE, fontweight='bold')
        ...
        if handles:
             ...
             # Move legend outside the plot area if many items, or upper right if few
             if len(by_label) > 8:
        ...
        # Add more padding at the bottom if the legend is placed there
        if len(by_label) > 8:
            plt.tight_layout(rect=[0, 0.1, 1, 0.95])  # Add space at the bottom
        else:
            plt.tight_layout()
