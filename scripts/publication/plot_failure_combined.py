#!/usr/bin/env python3
"""Generate two-panel broker-failure figure for the Embarcadero paper (Fig3).
...
def load_run(run_dir: str):
    ...
    try:
        ...
        for _, row in ev.iterrows():
            desc = str(row.get("EventDescription", "")).lower()
            t = row["t"]
            if ("broker kill" in desc or "kill requested" in desc) and events["kill"] is None:
                events["kill"] = t
...
def make_figure(nohold_dir: str, hold_dir: str):
    plt.rcParams.update(
        {
            "font.family": "serif",
            "font.size": 8,
    ...
    panels = [
        (nohold_dir, "(a) Holding disabled (arrival order)"),
        (hold_dir, "(b) Prefix-safe hold (ORDER=5)"),
    ]

    fig, axes = plt.subplots(1, 2, figsize=(6.8, 2.9), sharey=True, constrained_layout=True)
    ...
    for ax, (run_dir, title) in zip(axes, panels):
        ...
        ax.set_xlim(0, float(x_sec.max()) * 1.03)
        ...
        if ev["kill"] is not None:
            ax.axvline(ev["kill"], color=KILL_COLOR, linestyle="--", linewidth=0.9, alpha=0.85, label="Kill", zorder=5)
        if ev["detect"] is not None:
            ax.axvline(ev["detect"], color=DETECT_COLOR, linestyle=":", linewidth=0.9, alpha=0.85, label="Detect", zorder=5)
        if ev["reroute"] is not None:
            ax.axvline(ev["reroute"], color=REROUTE_COLOR, linestyle="-.", linewidth=0.9, alpha=0.85, label="Reroute", zorder=5)

        if ev["kill"] is not None and ev["reroute"] is not None:
            gap = ev["reroute"] - ev["kill"]
            mid = (ev["kill"] + ev["reroute"]) / 2
            ypos = data["Total_GBps"].max() * 0.10
            ax.annotate(f"{gap*1000:.0f}\u202fms", xy=(mid, ypos), fontsize=6, ha="center", color="#555555")
    ...
    # Optional backlog-flush callout on hold panel when peak is clipped.
    try:
        data_h, _xh, _bc, ev_h = load_run(hold_dir)
        peak = float(data_h["Total_GBps"].max()) if "Total_GBps" in data_h.columns else 0.0
        if peak > YMAX_CAP_GBPS and ev_h.get("reroute") is not None:
            axes[1].annotate(
                f"backlog\nflush\n({peak:.0f} GB/s)",
                xy=(ev_h["reroute"] + 0.03, YMAX_CAP_GBPS * 0.995),
                xytext=(ev_h["reroute"] + 0.50, YMAX_CAP_GBPS * 0.62),
                fontsize=5.5,
                color="#555555",
                ha="left",
                va="top",
                arrowprops=dict(arrowstyle="-|>", color="#555555", lw=0.7, connectionstyle="arc3,rad=0.15"),
            )
    except Exception:
        pass
    ...
    fig.legend(handles, labels, loc="lower center", ncol=5, bbox_to_anchor=(0.5, -0.18), frameon=True, handlelength=1.8)
    return fig


def main(argv=None):
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--nohold-dir", default="", help="Panel (a) arrival-order / hold-disabled run dir")
    p.add_argument("--hold-dir", default="", help="Panel (b) prefix-safe hold run dir")
    p.add_argument(
        "--out",
    ...
    nohold = args.nohold_dir or os.path.join(LEGACY_RUNS, "20260402_c4_order4_matchdiag")
    hold = args.hold_dir or os.path.join(LEGACY_RUNS, "20260402_c4_order5_acked_blogdiag")
