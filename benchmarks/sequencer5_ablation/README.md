...
Standalone sequencer implementation and benchmark for OSDI/SOSP/SIGCOMM-level ablation: correctness-first dedup, MPSC ring (multiple producers per broker), three-layer gap resolution, and **per-batch vs per-epoch atomic** comparison. Aligns with **docs/design/EMBARCADERO_DEFINITIVE_DESIGN.md**.
