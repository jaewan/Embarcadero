...
**Token-acquisition order restored (2026-07-16, `[[CORFU_FIFO_FIX]]`):** the shared-publisher path used to issue `GetTotalOrder` from parallel per-broker send threads, so one client's cross-broker token grants followed RPC arrival order — measurably permuting submission order (SMR-FIFO eval: ~2 inverted batch pairs / 60 batches, session-FIFO Valid=NO). That violated the frozen protocol's own promise ("total_order a valid shuffle of per-client submission order across brokers", §5.1). Fix is client-behavior-only in the shared publisher (Track-01 file, flagged for the eventual Track-03 fork): each batch's seal-time global `batch_seq` acts as a token-issuance ticket, so a batch's token request is held until every earlier-sealed batch's grant returns. No message, field, exchange-pattern, or sequencer change; token-before-write and the 2-RTT client path preserved. Post-fix the SMR KV eval shows Corfu Pipe Valid=YES with 0 reorders at 483K ops/s (down from an unfaithful 819K — the serialized token-acquisition cost E1 is meant to expose).
