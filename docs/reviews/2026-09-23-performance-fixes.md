...
A subsequent [ordered-consumer wait investigation](2026-09-23-ordered-wait-followup.md) removes 50 µs idle polling, checks both client sanitizer fixtures, and compares frozen source4 with source5 against the same broker. Its separate four-pair trial is inconclusive on throughput and CPU effect; it is not pooled with this whole-stack pilot.
