...
The fault profile requires 72 GiB free in `/dev/shm`, broker-node memory checked by the shared preflight, and 6 GiB free on the client node. It keeps 256 MiB segments and the full production GOI; storage cases use a 64-slot PBR derived from the native driver's side-effect-free geometry query and stay below 768 MiB of publisher ingress payload. Subscriber replay is additional traffic. After broker startup, an owned 60-second timer covers the entire active case, including blocked controller and native-stage waits; it is canceled before the separately bounded 15-second cleanup. Hooks use inherited local `SOCK_SEQPACKET` endpoints with per-run tokens, never public control listeners, and compile out of default builds. Fake-process harness tests run with `python3 -m unittest discover -s tools -p test_production_faults.py`; the native `--self-test` exercises wire transport, malformed boundaries and truncated frames without a cluster.
