...
- ~~D2 `session_lease_ns` blocks E~~ — **NOT a blocker (2026-07-07):** implement/test E with the env-overridable placeholder (1000/2000ms, both `>12ms`). D2's production value is a follow-up that must still satisfy `kDeltaCapMs(12) < session_lease_ms < PBR_fill_time_ms` AND `session_lease_ns >= open-load append/ack p99.9` — flag it, don't wait on it.
