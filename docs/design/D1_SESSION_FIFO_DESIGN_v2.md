...
> **IMPLEMENTATION STATUS (E-review 2026-07-07): R1 is DEFERRED FROM E TO F — still OPEN, not resolved.** Commit E (`a1b498e`) ships the fencing decision path but `RecoverSequencer5State` is **absent** (only an inline `ReconstructClientStateFromSessionEntry` seed from the volatile cache exists — no GOI-scan MAX leg, no election barrier, no park). This is **inert in E** (E drives no failover), but once Commit F wires reconnect/recovery it becomes a live re-accept hole in the publish-lost crash window (GOI committed but `SessionEntry.state_word` publish not durable ⇒ `next_expected` seeds too low ⇒ a retransmitted already-committed batch passes `seq < next_expected`). **F MUST land `RecoverSequencer5State` = MAX(GOI-scan+1, SessionEntry.expected_seq) with the park barrier (R-B: bounded park + NACK-on-overflow, never drop).** The §7 waterfall row that lists R1 under E is superseded by this deferral.
