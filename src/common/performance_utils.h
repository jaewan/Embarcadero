...
/**
 * @brief Chain role for `local_broker_id` when replicating batches from `source_broker_id`.
 * @return replica index in [0, replication_factor) if this broker participates, else -1.
 */
inline int GetReplicationChainRole(
		int local_broker_id, int source_broker_id, int replication_factor, int num_brokers) {
	if (source_broker_id < 0 || source_broker_id >= num_brokers || replication_factor <= 0 ||
	    num_brokers <= 0 || local_broker_id < 0 || local_broker_id >= num_brokers) {
		return -1;
	}
	for (int i = 0; i < replication_factor; ++i) {
		if (GetReplicationSetBroker(source_broker_id, replication_factor, num_brokers, i) == local_broker_id) {
			return i;
		}
	}
	return -1;
