...
class Client{
	public:
		...
		void Init(int num_threads, int msg_copy, char topic[TOPIC_NAME_SIZE], int ack_level, int order, size_t message_size){
			...
			size_t queueSize = total_queue_size_ / num_threads;

			ack_thread_ = std::thread([this](){
					this->EpollAckThread();
				});
			pubQues_.reserve(num_threads);
		...
		void Publish(std::string &message){
			static int i = 0;
			while(!pubQues_[i]->write(message.data())){
	...
	private:
		...
		void PublishThread(int threadIdx){
			int broker_id = threadIdx % nodes_.size();
			...
			header.next_msg_diff = nullptr;
			std::optional<char*> optReq;

			int JAE_DEBUG_sent_messages = 0;
			while(!shutdown_){
				while(!pubQues_[threadIdx]->read(optReq)){
					std::this_thread::yield();
				}
				if(!optReq.has_value()){
					VLOG(3) << "[JAE_DEBUG] " << threadIdx << " read:" << JAE_DEBUG_sent_messages << " que size:" << pubQues_[threadIdx]->sizeGuess();
					return;
				}
				JAE_DEBUG_sent_messages++;
...
void ThroughputTest(size_t total_message_size, size_t message_size, int num_threads, int ack_level, int order){
	...
	Client c("127.0.0.1", std::to_string(BROKER_PORT), 10000);
