...

	{
		...
		for (const auto& node_entry : nodes_) {
			...
			if (known_nodes.contains(broker_id)) {
				...
				// Backward compat: new_nodes still only lists "new" nodes for old clients
				initial_status.add_new_nodes(node_entry.second.network_mgr_addr);
			}
		}
	}

	// [[DIAGNOSTIC: Log broker_info being sent to client]]
	LOG(INFO) << "SubscribeToCluster: Sending initial status with " << initial_status.broker_info_size() << " brokers:";
	for (int i = 0; i < initial_status.broker_info_size(); i++) {
		const auto& bi = initial_status.broker_info(i);
		LOG(INFO) << "  B" << bi.broker_id() << ": accepts_publishes=" << bi.accepts_publishes()
		          << ", addr=" << bi.network_mgr_addr();
	}
