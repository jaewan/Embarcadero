...
Entry* FindOrClaimSession(Entry* table, size_t capacity, uint64_t key,
  ...
  for (size_t probe = 0; probe < capacity; ++probe) {
    ...
    if (seen == 0 && (entry->session_key.compare_exchange_strong(
                         seen, key, std::memory_order_acq_rel,
                         std::memory_order_acquire) || seen == key)) return entry;
  }
  return nullptr;
}
