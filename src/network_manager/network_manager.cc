...
using network::DurableSessionSnapshot;
static DurableSessionSnapshot ReadDurableSessionSnapshot(
    ...
    return network::ReadSessionSnapshot(cxl_manager->GetSessionTable(topic),
...
#if defined(EMBARCADERO_ENABLE_FAULT_INJECTION) && EMBARCADERO_ENABLE_FAULT_INJECTION
            if (to_read != 0 && !fault::Pause("ingress.payload.partial",
                    {handshake.client_id, connection_session_epoch, batch_header.batch_seq,
                     read, batch_header.total_size}, &stop_threads_)) {
                running = false;
                break;
            }
#endif
